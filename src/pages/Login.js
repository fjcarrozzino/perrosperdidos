...
import DarkImage from '../assets/georges-malher-XlknK9HfBlg-unsplash.jpg'
import DayImage from '../assets/ryan-stone-cL5THZBD5YQ-unsplash.jpg'
...
const Login = () => {
  ...
  const logIn = async (res) => {
    ...
    if (userInfo?.sub === usuarioLogin[0]?.userId) {
      ...
      await setDoc(doc(db, "users", randomId), valuesToSave);
      console.log("Usuario guardado");
    }

    if (res.credential) {
      localStorage.setItem("token", res.credential);
      dispatch(login(jwtDecode(res.credential)));
      setTimeout(() => {
        navigate("/");
      }, 500);
    } else {
          ...
          >
            ...
            <Box component="footer" sx={{ py: 3 }}>
              <Typography level="body-xs" textAlign="center">
                © Find my pet {new Date().getFullYear()}
              </Typography>
            </Box>
          </Box>
        </Box>
